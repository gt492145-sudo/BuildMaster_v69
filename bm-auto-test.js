(async function BM_AutoTest() {
    ...
    function canRunByOwnerGuard() {
        try {
            ...
            var ownerHash = localStorage.getItem('cm_owner_lock_hash_v1');
            if (ownerHash) {
                var unlocked = sessionStorage.getItem('cm_owner_lock_unlocked_v1') === '1';
                if (!unlocked) {
                    window.alert('請先解鎖管理者猴子權限，再執行自動測試。');
                    return false;
                }
            }
            return true;
